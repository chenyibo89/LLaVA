...
class MplugOwlVisualAbstractorModel(MplugOwlPreTrainedModel):
    def __init__(self, config: MplugOwlVisualAbstractorConfig, language_hidden_size):
        ...
        modules = [nn.Linear(config.hidden_size, language_hidden_size)]
        modules.append(nn.GELU())
        modules.append(nn.Linear(language_hidden_size, language_hidden_size))
        self.mlp = nn.Sequential(*modules)

        # self.mlp = nn.Sequential([nn.GELU(), nn.Linear(config.hidden_size, config.hidden_size)])
        self.post_init()
