...
from .clip_sam_encoder import CLIPSamVisionTower


def build_vision_tower(vision_tower_cfg, **kwargs):
    vision_tower = getattr(vision_tower_cfg, 'mm_vision_tower', getattr(vision_tower_cfg, 'vision_tower', None))
    # if vision_tower.startswith("openai") or vision_tower.startswith("laion"):
    if "clip-sam" in vision_tower.lower():
        print("use both clip and sam to build vision tower")
        return CLIPSamVisionTower(vision_tower, args=vision_tower_cfg, **kwargs)
    elif "clip-vit" in vision_tower.lower() and "chinese" in vision_tower.lower():
